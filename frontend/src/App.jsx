...
function formatApiError(error) {
  if (error?.code === "ECONNABORTED") return "The investigation timed out. Please retry.";
  const detail = error?.response?.data?.detail;
  if (Array.isArray(detail)) {
    const messages = detail.map((item) => typeof item === "object" && item !== null ? item.msg || item.message || JSON.stringify(item) : String(item)).filter(Boolean);
    if (messages.length) return messages.join("; ");
  }
  if (detail && typeof detail === "object") return detail.msg || detail.message || JSON.stringify(detail);
  if (typeof detail === "string" && detail.trim()) return detail;
  return error?.response ? "The investigation service returned an error." : "Unable to connect to the investigation service.";
}
